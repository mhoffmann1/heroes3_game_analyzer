...
class StatsPlugin(object):
    ...
    def __init__(self, parent, panel, version):
        ...
        self._state  = h3tools.hero.Attributes.factory(version)
        self._hero   = None
    ...
    def props(self):
        ...
        for prop in DATAPROPS:
            ...
            result.append(prop)
        return h3tools.version.adapt("hero.stats.DATAPROPS", result, version=self.version)
    ...
    def on_change(self, prop, row, ctrl, value):
        ...
        if "spellbook" == prop["name"]:
            evt = h3tools.gui.PluginEvent(self._panel.Id, action="render", name="spells")
    ...
    def on_experience_level(self, plugin, prop, state, event=None):
        ...
        if value == self._state[TARGET]:
            h3tools.guibase.status("%s already matching %s %s", prop["label"].capitalize(),
        ...
        def on_do(self, state):
            ...
            self.parent.patch()
            evt = h3tools.gui.PluginEvent(self._panel.Id, action="render", name=self.name)
            wx.PostEvent(self._panel, evt)
            return True
        label = "%s stats: %s %s" % (self._hero.name, TARGET, value)
        h3tools.guibase.status("Setting %s from %s", label, source_prop["label"].lower(),
