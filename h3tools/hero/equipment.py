...
class EquipmentPlugin(object):
    ...
    def __init__(self, parent, panel, version):
        ...
        self._state  = h3tools.hero.Equipment.factory(version)
        self._hero   = None
    ...
    def props(self):
        ...
        return h3tools.version.adapt("hero.equipment.DATAPROPS", result, version=self.version)
    ...
    def render(self):
        ...
        if self._ctrls and all(self._ctrls.values()): # All built and still valid
            ...
            self._ctrls, result = h3tools.gui.build(self, self._panel), True
        self.update_slots()
        return result
    ...
    def on_change(self, prop, row, ctrl, value):
        ...
        if stats0 != self._hero.stats:
            evt = h3tools.gui.PluginEvent(self._panel.Id, action="render", name="stats")
