...
class InventoryPlugin(object):
    ...
    def __init__(self, parent, panel, version):
        ...
        self._state  = h3tools.hero.Inventory.factory(version)
        self._hero   = None
    ...
    def compact_items(self, order=(), reverse=False):
        ...
        if items == self._state:
            h3tools.guibase.status("No change from compacting inventory",
        ...
        h3tools.guibase.status("Compacting inventory in %s order", label,
    ...
    def render(self):
        ...
        if self._ctrls and all(self._ctrls):
            ...
            self._ctrls = h3tools.gui.build(self, self._panel)[0]
