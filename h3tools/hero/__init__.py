...
class DataClass(object):
    ...
    def factory(cls, version):
        return h3tools.version.adapt("hero.%s" % cls.__name__, cls, version)()
...
class Army(TypedArray, DataClass):
    ...
    def __init__(self):
        ...
        dataclass = h3tools.version.adapt("hero.%s" % ArmyStack.__name__, ArmyStack, version)
...
class Attributes(SlotsDict, DataClass):
    ...
    def get_experience_level(self):
        ...
        EXP_LEVELS = h3tools.version.adapt("experience_levels", metadata.EXPERIENCE_LEVELS,
                                         version=self.get_version())
...
class Skills(TypedArray, DataClass):
    ...
    def __init__(self):
        version = self.get_version()
        dataclass = h3tools.version.adapt("hero.%s" % Skill.__name__, Skill, version)
