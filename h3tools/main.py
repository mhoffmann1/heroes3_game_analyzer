...
def run_info(filenames):
    ...
    for filename in filenames:
        ...
        try: savefile = h3tools.metadata.Savefile(filename, parse_heroes=False)
        except Exception as e:
            output("\nError reading %s: %s" % (filename, e))
            continue # for filename

        savefile.populate_heroes()
        content = yaml.safe_dump(h3tools.templates.make_savefile_data(savefile), sort_keys=False)
        output()
        output(content)
...
def run_export(filenames, format, outname=None, search=()):
    ...
    for filename in filenames:
        ...
        try: savefile = h3tools.metadata.Savefile(filename)
        except Exception as e:
            output("\nError reading %s: %s" % (filename, e))
            continue # for filename
        ...
        h3tools.templates.export_heroes(outfile, format, heroes, savefile)
